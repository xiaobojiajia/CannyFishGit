...
DataReader = {}
DataReader.TAT_DESC		  = 1 --描述栏
DataReader.TAT_NAMELINE   = 2 --标题栏
DataReader.TAT_FORMATLINE = 3 --格式栏
DataReader.MAIN_KEY		  = 1 --主键在的列

DataReader.TYPE_STRING    = "string"
DataReader.TYPE_NUMBER 	  = "number"
DataReader.TYPE_EQUATION  = "equation"
DataReader.TYPE_POINT	  = "point"
DataReader.TYPE_ARRAY	  = "array" --serialize
DataReader.TYPE_TABLE	  = "table" --key-value
...
function DataReader.parseEquation(str,paramName,paramTable)
	local number = 0
...
function DataReader.parsePoint(str)
	local x = 0
	local y = 0
	if str and string.len(str)>2 then
		...
		x=DataReader.parseEquation(p[1])
		y=DataReader.parseEquation(p[2])
	end
	return ccp(x,y)
end

function DataReader.parseTable(str)
	if str and string.len(str)>2 then
		local pstart,len=string.find(str, "{")
		local pend,len=string.find(str, "}")
		local valueStr=string.sub(str, pstart+1, pend-1)
...
function DataReader.read(fileName)
	printf("fileName",fileName)
  	assert(fileName,"DataReader.read fileName is null" ..fileName)
  	local lineTable_={}
  	local dataTile_={}
	local dataFormat_={}
	local data_={}
	...
	for i,v in ipairs(lineTable_) do
		...
		if i == DataReader.TAT_DESC then
			
		elseif i == DataReader.TAT_NAMELINE then
			dataTile_=elementTable_
		elseif i==DataReader.TAT_FORMATLINE then
			dataFormat_=elementTable_
		else 
			--先解析除了公式外的所有数据
			for ei,ev in ipairs(elementTable_) do
				local dataType_ = string.lower(dataFormat_[ei])
				if dataType_ == DataReader.TYPE_STRING then
					linedata_[dataTile_[ei]] = tostring(ev)
				elseif dataType_ == DataReader.TYPE_NUMBER then
					linedata_[dataTile_[ei]] = tonumber(ev)
				elseif dataType_ == DataReader.TYPE_POINT then
					linedata_[dataTile_[ei]] = DataReader.parsePoint(string.trimsymbol(ev))
				elseif dataType_ == DataReader.TYPE_ARRAY then

					linedata_[dataTile_[ei]] = string.split(string.trimsymbol(ev),",")
				elseif dataType_ == DataReader.TYPE_TABLE then
					linedata_[dataTile_[ei]] = DataReader.parseTable(ev)
				end
			end
			--仅仅解析公式模式
			for ei,ev in ipairs(elementTable_) do
				if dataFormat_[ei] == DataReader.TYPE_EQUATION  then
					linedata_[dataTile_[ei]] = DataReader.parseEquation(ev,dataTile_,linedata_)
				end
			end 
			data_[elementTable_[DataReader.MAIN_KEY]]=linedata_
		end
	end
	return data_
end
